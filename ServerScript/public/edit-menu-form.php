...
<div id="content" class="container col-md-12">
	<?php 
		...
		if(isset($_POST['btnEdit'])){
			...
			if( !empty($news_heading) && 
				...
				// check update result
				if($update_result){
					$error['update_data'] = " <span class='label label-primary'>Success update recipe.</span>";
				}else{
					$error['update_data'] = " <span class='label label-danger'>Failed to update recipe.</span>";
				}
			}
		...
		$sql_query = "SELECT * FROM tbl_recipes WHERE nid = ?";
	...
	?>
	<div class="col-md-12">
	<h1>Edit Recipe <?php echo isset($error['update_data']) ? $error['update_data'] : '';?></h1>
	<hr />
	</div>
	<form method="post" enctype="multipart/form-data">
	<div class="col-md-10">
		<div class="col-md-4">
	    <label>Recipe Name :</label><?php echo isset($error['news_heading']) ? $error['news_heading'] : '';?>
		<input type="text" name="news_heading" class="form-control" value="<?php echo $data['news_heading']; ?>"/>
		<br/>
	    <label>Cooking Time :</label><?php echo isset($error['news_date']) ? $error['news_date'] : '';?>
